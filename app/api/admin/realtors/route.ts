...
export async function GET(request: Request) {
  try {
    ...
    const snap = await adminDb().collection("realtors").orderBy("createdAt", "desc").limit(150).get();
...
export async function PATCH(request: Request) {
  try {
    ...
    const { realtorId, verificationStatus, creditAdjustment, note } = await request.json();
    ...
    await db.runTransaction(async tx=>{
      ...
      if(verificationStatus!==undefined){
        ...
        patch.subscriptionStatus=verificationStatus==="verified"?(r.subscriptionStatus==="pending_verification"?"beta_active":r.subscriptionStatus):verificationStatus==="rejected"?"suspended":r.subscriptionStatus;
        if(verificationStatus==="verified" && r.verificationStatus!=="verified" && !r.creditsGrantedAt && appConfig.defaultBetaCredits>0){
          ...
          ledger={id:ledgerRef.id,realtorId:r.id,realtorUid:r.uid,amount:appConfig.defaultBetaCredits,balanceAfter,type:"verification_bonus",referenceId:r.id,note:"Initial beta test credits",createdAt:now};
      ...
      auditPayload={verificationStatus,creditAdjustment,note}; tx.update(ref,patch);
    });
