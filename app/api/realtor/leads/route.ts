...
export async function GET(request: Request) {
  try {
    ...
    const realtorPublic = {
      id: realtor.id, fullName: realtor.fullName, agencyName: realtor.agencyName, country: realtor.country || "PK", city: realtor.city,
      ...
      planLeadCap: Number(realtor.planLeadCap || 0), planLeadsUsed: Number(realtor.planLeadsUsed || 0),
      planPeriodStart: realtor.planPeriodStart || "", planPeriodEnd: realtor.planPeriodEnd || "",
      paygLeadBalance: Number(realtor.paygLeadBalance || 0),
      billingMode: realtor.billingMode || process.env.BILLING_MODE || "beta_credits",
    };
    if (user.email_verified !== true) return NextResponse.json({ ok: true, pendingEmailVerification: true, realtor: realtorPublic, leads: [] });
    if (realtor.status !== "active") return NextResponse.json({ ok: true, suspended: true, realtor: realtorPublic, leads: [] });
    ...
    const leads = leadsSnap.docs.map(d => d.data()).filter((l: any) =>
      l.verificationStatus === "verified" && l.status === "verified" && (l.country || "PK") === (realtor.country || "PK") &&
      ...
      id: l.id, country: l.country || "PK", type: l.type, city: l.city, area: l.area, propertyType: l.propertyType,
