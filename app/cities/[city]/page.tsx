...
export default async function CityPageRoute({params}:{params:Promise<{city:string}>}){
  ...
  return <main><Nav/>
    <section className="cityHero hasHeroArtwork"><div className="cityNumber">{city.number}</div><p className="eyebrow">BLUEPRINT BUDDIES / {city.name.toUpperCase()}</p><h1>{city.name.toUpperCase()}<br/><span>DEMAND.</span></h1><p>{city.intro}</p><HeroArtwork src={`/images/heroes/${city.slug}.webp`} alt={`${city.name} property market and neighborhoods`} priority/></section>
