...
export function AdminDashboard(){
  ...
  return <div className="portalShell adminPortal">
    ...
    <main className="adminShell">
      ...
      {loading?<div className="portalLoading"><span/><p>Loading operations…</p></div>:tab==="realtors"?<section><div className="sectionLine"><div><p className="eyebrow">ACCOUNT ACCESS</p><h2>REALTORS</h2></div><span>{filteredRealtors.length} SHOWN / {queueCount} PENDING</span></div><div className="adminTable">{filteredRealtors.map(r=><article className="adminRow" key={r.id}><div><strong>{r.fullName}</strong><span>{r.agencyName} · {r.city}</span><small>{r.areas?.join(" · ")||"No areas selected"}</small></div><span className={`statusPill status-${r.verificationStatus}`}>{r.verificationStatus}</span><div>{r.verificationStatus==="pending"?<><button disabled={busy===r.id} onClick={()=>verify(r.id,"verified")}>VERIFY</button><button disabled={busy===r.id} className="dangerButton" onClick={()=>verify(r.id,"rejected")}>REJECT</button></>:<span className="adminResolved">NO ACTION REQUIRED</span>}</div></article>)}</div>{!filteredRealtors.length&&<div className="emptyBox"><strong>{actionOnly?"REALTOR QUEUE IS CLEAR.":"NO REALTORS MATCH."}</strong><p>{actionOnly?"There are no pending realtor applications requiring review.":"Try a different search."}</p></div>}</section>:<section><div className="sectionLine"><div><p className="eyebrow">CONTACT QUALITY</p><h2>LEADS</h2></div><span>{filteredLeads.length} SHOWN / {queueCount} PENDING</span></div><div className="adminTable">{filteredLeads.slice(0,100).map(l=><article className="adminRow" key={l.id}><div><strong>{l.type?.toUpperCase()} · {l.area}</strong><span>{l.name} · {l.phone}</span><small>{l.propertyType} · {l.timeframe} · {l.city}</small></div><span className={`statusPill ${l.temperature}`}>{l.temperature} / {l.verificationStatus}</span><div>{l.verificationStatus!=="verified"?<button disabled={busy===l.id} onClick={()=>verifyLead(l.id)}>VERIFY CONTACT</button>:<span className="verifiedLabel">✓ VERIFIED</span>}</div></article>)}</div>{!filteredLeads.length&&<div className="emptyBox"><strong>{actionOnly?"LEAD QUEUE IS CLEAR.":"NO LEADS MATCH."}</strong><p>{actionOnly?"There are no lead contacts waiting for verification.":"Try a different search."}</p></div>}</section>}
    </main>
  </div>
}
