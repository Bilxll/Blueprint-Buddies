    ...
    <main className="dashboardShell">
      ...
      {loading?<div className="portalLoading"><span/><p>Loading your territory…</p></div>:pending?<div className="pendingBox"><p className="eyebrow">VERIFICATION PENDING</p><h2>YOUR MARKETPLACE<br/><span>ACCESS IS LOCKED.</span></h2><p>We review realtor profiles before exposing property opportunities or consumer contact information. You can return to this portal after approval.</p></div>:<>
        ...
        {tab==="discover"&&<section className="dashboardSection">
          ...
          <div className="leadToolbar"><label><Search size={17}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search area or property type"/></label><div><SlidersHorizontal size={16}/><select value={type} onChange={e=>setType(e.target.value)} aria-label="Lead type"><option value="all">ALL TYPES</option><option value="buy">BUYERS</option><option value="sell">SELLERS</option><option value="invest">INVESTORS</option><option value="rent">RENTALS</option></select><select value={temp} onChange={e=>setTemp(e.target.value)} aria-label="Lead intent"><option value="all">ALL INTENT</option><option value="hot">HOT</option><option value="warm">WARM</option><option value="future">FUTURE</option><option value="needs_verification">NEEDS VERIFY</option></select><select value={sort} onChange={e=>setSort(e.target.value)} aria-label="Sort opportunities"><option value="recommended">HOT FIRST</option><option value="newest">NEWEST</option><option value="least_claimed">LEAST CLAIMED</option></select>{hasFilters&&<button className="filterReset" onClick={()=>{setType("all");setTemp("all");setSort("recommended");setQuery("")}}>RESET</button>}</div></div>
          <div className="leadGrid">{filtered.map(l=>{const remaining=Math.max(0,(l.maxClaims||3)-(l.claimCount||0));return <article className="leadCard" key={l.id}><div className="leadTop"><span className={`temp ${l.temperature}`}><Flame size={14}/>{l.temperature.replace("_"," ")}</span><span>{daysAgo(l.createdAt)}</span></div><div className="leadMatchTag">YOUR TERRITORY · {l.area}</div><div className="leadLocation"><span>{l.type.toUpperCase()}</span><h3>{l.area}<br/><b>{l.city}</b></h3></div><div className="leadMeta"><p><MapPin size={15}/>{l.propertyType} · {l.size||"Size open"}</p><p className="leadBudget">{moneyRange(l)}</p><p>{l.paymentMode||"Payment open"} · {l.timeframe}</p><p className={l.verificationStatus==="verified"?"verifiedText":"mutedText"}>{l.verificationStatus==="verified"?"✓ CONTACT VERIFIED":"CONTACT NOT YET VERIFIED"}</p></div><div className="claimAvailability"><span>{remaining}</span> claim slot{remaining===1?"":"s"} remaining</div><button disabled={busyId===l.id||remaining===0} className="claimBtn" onClick={()=>claim(l.id)}>{busyId===l.id?"CLAIMING…":"CLAIM OPPORTUNITY"}<ArrowUpRight size={16}/></button></article>})}</div>
