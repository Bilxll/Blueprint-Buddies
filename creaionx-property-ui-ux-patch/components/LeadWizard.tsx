...
import { ArrowLeft, ArrowRight, Check, Flame, Home, BadgeDollarSign, TrendingUp, KeyRound, LockKeyhole } from "lucide-react";
...
export function LeadWizard(){
 ...
 if(done)return <div className="successPanel"><div className="successIcon"><Check/></div><p className="eyebrow">REQUIREMENT RECEIVED</p><h2>YOUR BRIEF IS<br/><span>IN THE NETWORK.</span></h2><p>Reference <strong>{done.id}</strong>. Your intent is currently marked <strong>{done.temperature.replace("_"," ").toUpperCase()}</strong>. Verification is handled separately before any verified label is shown to realtors.</p><div className="successActions"><a className="ctaButton inlineButton" href="/"><span className="ctaFill"/><span className="ctaText">BACK HOME <ArrowRight size={18}/></span></a><a className="secondaryLink" href="/get-started">SUBMIT ANOTHER <ArrowRight size={18}/></a></div></div>;
 ...
 return <div className="wizardShell">
  <div className="wizard">
    ...
    {step===0&&<section className="wizardStage"><p className="eyebrow">YOUR MOVE</p><h1>WHAT ARE YOU<br/>HERE TO DO?</h1><div className="typeGrid">{TYPES.map((t,i)=>{const Icon=t.icon;return <button type="button" key={t.id} className={`typeChoice ${data.type===t.id?"active":""}`} onClick={()=>patch("type",t.id)}><div><span>0{i+1}</span><Icon size={21}/></div><strong>{t.label}</strong><small>{t.kicker}</small></button>})}</div></section>}

    {step===1&&<section className="wizardStage"><p className="eyebrow">LOCATION</p><h1>WHERE IS THE<br/>PROPERTY?</h1><div className="formGrid"><label>CITY<select className={inputClass} value={data.city} onChange={e=>{patch("city",e.target.value);patch("area",CITY_AREAS[e.target.value]?.[0]||"Other")}}>{Object.keys(CITY_AREAS).map(c=><option key={c}>{c}</option>)}</select></label><label>AREA / SOCIETY<select className={inputClass} value={data.area} onChange={e=>patch("area",e.target.value)}>{areas.map(a=><option key={a}>{a}</option>)}</select></label><label>PROPERTY TYPE<select className={inputClass} value={data.propertyType} onChange={e=>patch("propertyType",e.target.value)}>{PROPERTY_TYPES.map(p=><option key={p}>{p}</option>)}</select></label><label>SIZE <small>OPTIONAL</small><input className={inputClass} placeholder="e.g. 500 sq yd / 10 marla" value={data.size||""} onChange={e=>patch("size",e.target.value)}/></label></div></section>}

    {step===2&&<section className="wizardStage"><p className="eyebrow">INTENT</p><h1>MAKE THE BRIEF<br/><span>USEFUL.</span></h1><div className="formGrid"><label>{data.type==="sell"?"EXPECTED PRICE (PKR)":"MIN BUDGET (PKR)"}<input className={inputClass} inputMode="numeric" type="number" min="0" value={data.type==="sell"?(data.expectedPrice||""):(data.budgetMin||"")} onChange={e=>patch(data.type==="sell"?"expectedPrice":"budgetMin",e.target.value)} placeholder="e.g. 20000000"/></label>{data.type!=="sell"&&<label>MAX BUDGET (PKR)<input className={inputClass} inputMode="numeric" type="number" min="0" value={data.budgetMax||""} onChange={e=>patch("budgetMax",e.target.value)} placeholder="e.g. 30000000"/></label>}<label>TIMEFRAME<select className={inputClass} value={data.timeframe} onChange={e=>patch("timeframe",e.target.value)}>{TIMEFRAMES.map(t=><option key={t}>{t}</option>)}</select></label><label>PAYMENT<select className={inputClass} value={data.paymentMode||""} onChange={e=>patch("paymentMode",e.target.value)}><option>Cash</option><option>Installments</option><option>Financing / Mortgage</option><option>Open to options</option></select></label>{["buy","rent"].includes(data.type)&&<label>BEDROOMS<select className={inputClass} value={data.bedrooms||"Open"} onChange={e=>patch("bedrooms",e.target.value)}><option>Open</option><option>1</option><option>2</option><option>3</option><option>4</option><option>5+</option></select></label>}{data.type!=="sell"&&<label>PURPOSE <small>OPTIONAL</small><input className={inputClass} placeholder={data.type==="invest"?"e.g. long-term investment":"e.g. family residence"} value={data.purpose||""} onChange={e=>patch("purpose",e.target.value)}/></label>}{data.type==="sell"&&<label className="checkLine"><input type="checkbox" checked={!!data.ownerConfirmed} onChange={e=>patch("ownerConfirmed",e.target.checked)}/><span>I am the owner or authorized seller</span></label>}{data.type==="invest"&&<label>INVESTMENT GOAL<select className={inputClass} value={data.investmentGoal||"Appreciation"} onChange={e=>patch("investmentGoal",e.target.value)}><option>Appreciation</option><option>Rental income</option><option>Both</option></select></label>}</div></section>}

    {step===3&&<section className="wizardStage"><p className="eyebrow"><Flame size={15}/> FINAL STEP</p><h1>WHERE SHOULD<br/>REALTORS REACH YOU?</h1><div className="formGrid"><label>FULL NAME<input required className={inputClass} autoComplete="name" value={data.name} onChange={e=>patch("name",e.target.value)} placeholder="Your full name"/></label><label>WHATSAPP / PHONE<input required className={inputClass} inputMode="tel" autoComplete="tel" value={data.phone} onChange={e=>patch("phone",e.target.value)} placeholder="03XX XXXXXXX"/></label><label>EMAIL <small>OPTIONAL</small><input className={inputClass} type="email" autoComplete="email" value={data.email} onChange={e=>patch("email",e.target.value)} placeholder="you@email.com"/></label></div><label className="consentLine"><input type="checkbox" checked={consent} onChange={e=>setConsent(e.target.checked)}/><span><strong>CONTACT PERMISSION</strong> I agree that authorized participating realtors matched to this requirement may contact me about it.</span></label><p className="privacyNote"><LockKeyhole size={16}/> Your phone and email are never displayed on the public marketplace. Contact details unlock only after an authorized realtor claims the opportunity.</p></section>}
